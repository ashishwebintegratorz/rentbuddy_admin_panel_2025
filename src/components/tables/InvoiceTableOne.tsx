...
export default function InvoiceTableOne() {
  ...
  // 🔥 Payment type filter: all / cumulative / recurring
  const [paymentFilter, setPaymentFilter] = useState<
    "all" | "cumulative" | "recurring"
  >("all");
  const [isPaymentDropdownOpen, setIsPaymentDropdownOpen] = useState(false);

  const paymentLabelMap: Record<
    "all" | "cumulative" | "recurring",
    string
  > = {
    all: "All Payments",
  ...
  const fetchInvoices = async () => {
    ...
    try {
      ...
      const mapped: InvoiceRow[] = data.map((inv: any) => {
        ...
        const fullNameRaw =
          `${firstName} ${lastName}`.trim() || inv.username || inv.userEmail;

        const name =
          fullNameRaw && fullNameRaw.length > 0
            ? fullNameRaw
            : inv.userEmail || "Unknown";
  ...
  }, [BASE_API_URL, page]); // 👈 fetch again on page change

  // reset page when search/payment filter changes (optional)
  useEffect(() => {
    setPage(1);
  ...
  const filtered = useMemo(() => {
    ...
    return invoices.filter((inv) => {
      ...
      const matchesPayment =
        paymentFilter === "all" ||
        (paymentFilter === "cumulative" &&
          payment === "cumulative payment") ||
        (paymentFilter === "recurring" &&

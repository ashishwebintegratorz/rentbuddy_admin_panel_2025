...
/* ---------------- SAFETY WRAPPER ---------------- */
function SafeChart({ data, children }: any) {
...
function buildDonut(labels: string[] = [], format: (v: number) => string): ApexOptions {
  return {
...
export function AvgOrderValueChart({ data }: { data: MonthlyRevenue[] }) {
  ...
  <SafeChart data={data}>
    <Chart
      type="line"
      height={250}
      series={[{ name:"Avg Order Value", data:data.map(d=>d.avgOrderValue||0) }]}
      options={{
        chart:{ toolbar:{show:false} },
        stroke:{ width:3, curve:"smooth" },
        markers:{ size:4 },
...
export function TopProductsChart({ data }: { data: ProductRevenue[] }) {
  const formatted = data?.map(i=>({name:i.productName||"Unknown", value:i.revenue||0})) || [];
...
export function TopCustomersChart({ data }: { data: TopCustomer[] }) {
  const formatted = data?.map(i=>({name:i.name||"Unknown", value:i.spent||0})) || [];
  return <RevenueBar data={formatted} title="Top Spending Customers" />;
}

/* 🔥 Common Safe Bar Component */
function RevenueBar({data,title}:{data:{name:string,value:number}[],title:string}) {
  return (
  <SafeChart data={data}>
    <Chart
      type="bar"
      height={280}
      series={[{name:"Revenue",data:data.map(d=>d.value||0) }]}
      options={{
        colors:["#10b981"],
        chart:{ toolbar:{show:false} },
        plotOptions:{ bar:{ horizontal:true, borderRadius:4 } },
        xaxis:{ categories:data.map(d=>d.name||""), labels:{formatter:(v)=>`₹${v}`}},
        tooltip:{ y:{ formatter:(v)=>`₹${v.toLocaleString("en-IN")}`}},
        dataLabels:{ enabled:false },
...
export function RevenueByState({ data }: { data: StateRevenue[] }) {
  ...
  <SafeChart data={data}>
    <Chart type="donut" height={240}
      ...
      options={buildDonut(data.map(d=>d.state||"NA"),(v)=>`₹${v}`)}
    />
  </SafeChart>
  );
}
...
export function PaymentTypeChart({ data }: { data: PaymentTypeBreakdown[] }) {
  ...
  <SafeChart data={data}>
    <Chart type="donut" height={240}
      ...
      options={buildDonut(data.map(d=>d.paymentType||"Unknown"),(v)=>`₹${v}`)}
    />
  </SafeChart>
  );
}
...
export function PaymentStatusChart({ data }: { data: PaymentStatusBreakdown[] }) {
  ...
  <SafeChart data={data}>
    <Chart type="donut" height={240}
      series={data.map(d=>d.amount||0)}
      options={buildDonut(data.map(d=>d.status||"Unknown"),(v)=>`₹${v}`)}
    />
  </SafeChart>
  );
}

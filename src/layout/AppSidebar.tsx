...
const navItems: NavItem[] = [
...

  {
    ...
    subItems: [
      { name: "Customers", path: "/customers", allowedRoles: ["admin", "customer manager"] },
      { name: "Complaints", path: "/complaints", allowedRoles: ["admin", "customer manager"] },
      { name: "Defaulters", path: "/defaulters", allowedRoles: ["admin", "customer manager"] },
      { name: "Rent History", path: "/rent-history", allowedRoles: ["admin", "customer manager"] },
    ],
  },
...

  {
    ...
    subItems: [
      { name: "Products", path: "/products", allowedRoles: ["admin", "product manager"] },
      { name: "Packages", path: "/packages", allowedRoles: ["admin", "product manager"] },
      { name: "Barcode", path: "/barcode", allowedRoles: ["admin", "product manager"] },
      { name: "Track", path: "/track", allowedRoles: ["admin", "product manager"] },
    ],
  },

  {
    ...
    subItems: [
      { name: "Payments", path: "/payments", allowedRoles: ["admin", "finance manager"] },
      { name: "Invoice", path: "/invoice", allowedRoles: ["admin", "finance manager"] },
      { name: "Documents", path: "/documents", allowedRoles: ["admin", "finance manager"] },
    ],
  },
];
...
const AppSidebar: React.FC = () => {
  ...
  const canAccess = (roles?: string[]) => !roles || roles.includes(userRole) || userRole === "admin";

  const [openSubmenu, setOpenSubmenu] = useState<{ type: "main" | "others"; index: number } | null>(null);
  const [subMenuHeight, setSubMenuHeight] = useState<Record<string, number>>({});
  const subMenuRefs = useRef<Record<string, HTMLDivElement | null>>({});

  const isActive = useCallback((p: string) => location.pathname === p, [location.pathname]);
  ...
  const toggle = (i: number, t: "main" | "others") =>
    setOpenSubmenu(prev => prev && prev.index === i && prev.type === t ? null : { type: t, index: i });

  const renderMenuItems = (items: NavItem[], type: "main" | "others") => (
    <ul className="flex flex-col gap-3">
      {items.filter(n => canAccess(n.allowedRoles)).map((nav, index) => (
        <li key={nav.name}>
          {nav.subItems ? (
            <button
              ...
              <span className={`menu-item-icon-size ${openSubmenu?.type === type && openSubmenu?.index === index ? "menu-item-icon-active" : "menu-item-icon-inactive"}`}>
                {nav.icon}
              </span>
              {(isExpanded || isHovered || isMobileOpen) && <span className="menu-item-text">{nav.name}</span>}
              {(isExpanded || isHovered || isMobileOpen) && (
                <ChevronDownIcon className={`ml-auto h-5 w-5 duration-200 ${openSubmenu?.type === type && openSubmenu?.index === index ? "rotate-180 text-brand-500" : ""}`} />
              )}
            </button>
          ) : (
            nav.path && (
          ...
          {nav.subItems && (isExpanded || isHovered || isMobileOpen) && (
            <div ref={el => { subMenuRefs.current[`${type}-${index}`] = el; }}
              ...
              <ul className="mt-2 ml-9 space-y-1">
                {nav.subItems.filter(s => canAccess(s.allowedRoles)).map((s: any) => (
                  <li key={s.name}>
      ...
      <div className="flex flex-1 flex-col overflow-y-auto no-scrollbar pb-6 pr-1">
        <nav className="mb-6">
          <div className="flex flex-col gap-5">
            <div>
              <h2 className={`mb-3 flex text-xs uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500 ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}>
